...
@if(isset($market->name))
    <div class="ui segment partial-coin-detail-wrapper">
      ...
      <div class="ui segment coin-detail-wrapper">
         ...
         <div class="ui label cd simple dropdown item" style="background:#f06060;color:#fff;"><i class="google wallet icon"></i> Get Wallet <i class="dropdown icon"></i>
          <div class="menu">
              @foreach($object_details->getCustomWallets($market->symbol)['custom_wallets'] as $wallet)
              <div class="item">
                  <a rel="nofollow" href="{{ $wallet->affiliate_link }}" target="_blank" title="Get Free {{$market->symbol}} Wallet" alt="Get Free {{$market->symbol}} Wallet">
                      {{ $wallet->wallet_name }}
                  </a>
                  <p><small>{{ $wallet->sub_text }}</small></p>
              </div>
              @endforeach
              @foreach($object_details->getCustomWallets($market->symbol)['all_wallets'] as $wallet)
              <div class="item">
                  <a rel="nofollow" href="{{ $wallet->affiliate_link }}" target="_blank" title="Get Free {{$market->symbol}} Wallet" alt="Get Free {{$market->symbol}} Wallet">
                      {{ $wallet->wallet_name }}
                  </a>
                ...
                <div class="item">
                   <h3 id="usd" class="ui header">
                      ...
                      <div class="content colored">
                         ...
                         <div id="usd" class="sub header price colored">@if($market->price_usd < 0.9999) $ {{ number_format($market->price_usd, 4) }} @else $ {{ number_format($market->price_usd, 2) }}@endif</div>
                      </div>
                   </h3>
                </div>
                <!-- Volume-->
        ...
        <div class="row">
            <div class="four wide column detail-page-market-stats">
                <div class="ui mini statistic">
                    <div class="label">@lang('tbl_headings.24h_VOLUME')</div><span id="dailyHigh" class="value price-fiat">@if(isset($market->price_usd) && $market->price_usd > 0) @if(isset($market->price_btc)){{ number_format(($market->volume_usd_day/$market->price_usd)*$market->price_btc) }} BTC @else --- @endif @else 0 @endif</span></div>
            </div>
            <div class="four wide column">
                <div class="ui mini statistic">
    ...
    <acronym title="Bitcoin">BTC</acronym></strong>. @lang('seo.COIN_DES4') <strong>@if($market->available_supply != 0) {{ $market->available_supply }} {{ $market->symbol }} @else N/A @endif </strong> @lang('seo.COIN_DES6') {{ $market->name }} @lang('seo.COIN_DES7') <strong> {{ $market->percent_change_day }} %</strong>@lang('seo.COIN_DES8') <a href="{{ makeUrl('currencies') }}" title="@lang('seo.COIN_DES9')">@lang('seo.COIN_DES9')</a> @lang('seo.COIN_DES10') <strong>{{ number_format($market->volume_usd_day/100000000, 3) }} M US dollars</strong> @lang('seo.COIN_DES11') <strong>{{ $market->name }}</strong> @lang('seo.COIN_DES12') <a href="{{ makeUrl('crypto-exchanges') }}" title="@lang('seo.COIN_DES13')">@lang('seo.COIN_DES13')</a>. @lang('v7.SEO_DESC14') {{ $market->name }}'s @lang('v7.SEO_DESC15') <a href="{{ makeUrl('user/favorites-coins') }}" title="Favorites Coins">@lang('v7.SEO_DESC16')</a> @lang('v7.SEO_DESC17') <a href="{{ makeUrl('user/blockfolio') }}" title="Portfolio">@lang('v7.SEO_DESC18')</a>.</div>
       <div class="ui divider"></div>
       <div style="margin-top: 20px; margin-bottom: 20px;" class="ui grid center aligned">
          ...
          <div class="col-md-2 col-sm-3 col-xs-4">
             <div class="info-box">
                ...
                <div class="info-text"> @if(isset($market->price_btc) && $market->price_btc > 0){{ number_format(($market->market_cap_usd/$market->price_usd)*$market->price_btc) }} BTC @else --- @endif </div>
             </div>
          </div>
          <div class="col-md-2 col-sm-3 col-xs-4">
             <div class="info-box">
                <div class="info-label">BTC 24h Vol</div>
                <div class="info-text"> @if(isset($market->price_usd) && $market->price_usd > 0) @if(isset($market->price_btc)){{ number_format(($market->volume_usd_day/$market->price_usd)*$market->price_btc) }} BTC @else --- @endif @else 0 @endif</div>
             </div>
          </div>
          <div class="col-md-2 col-sm-3 col-xs-4">
             <div class="info-box">
    ...
    <div class="row">
      ...
      @if((isset($coin_details->twitter) && $coin_details->twitter != '') || (isset($coin_details->reddit) && $coin_details->reddit !=''))
      ...
      <div class="col-md-2">
         <div class="info-box">
            <a href="{{ URL::to('/currencies') }}/{{$coin['alias']}}" class="media-left" style="display: block;">
              ...
              <img alt="{{$coin['name']}}" title="{{$coin['name']}}" src="{{URL::asset('public/storage/' . $coin['image'])}}" height="180" style="display: block;margin-right: auto;margin-left: auto;">
              @else
              <img alt="{{$coin['name']}}" title="{{$coin['name']}}" src="{{ str_replace('/thumbs', '', $coin['image']) }}" height="180" style="display: block;margin-right: auto;margin-left: auto;"> 
              @endif
            </a>
