...
<div class="ui segment">
    ...
    <div class="row">
        <div class="col-md-5 pool-detail">
            ...
            @else
            <img class="img-fluid" src="{{URL::asset('public/images/coins_icons/' . $ico->coin_image)}}" title="{{ $ico->name }}" alt="{{ $ico->name }}" style="width: 220px;">
            @endif
        </div>
    </div><br />
    @endif
    ...
    @if($ico->affiliate != '')
    <a href="{{$ico->affiliate}}" target="_blank" title="{{ $ico->name }}" rel="nofollow noopener noreferrer">
...
<div class="box box-success table-heading-class">
    ...
    <p style="margin: 10px;">@lang('v7.RELATED_ICOS_DESC')<br />&nbsp;</p>
</div>
<div class="row">
    @foreach ($icos as $ico)
        @if(file_exists('public/storage/' . $ico->image) || file_exists('public/images/coins_icons/' . $ico->coin_image))
        <div class="col-4 col-xl-2 col-lg-2 col-md-3 col-sm-4 mb-3">    
            <a class="card" href="{{ makeUrl('crypto-ico') }}/{{ $ico->alias }}" title="{{ $ico->name }}">  
                @if(file_exists('public/storage/' . $ico->image))
                <img class="img-fluid" src="{{URL::asset('public/storage/' . $ico->image)}}" title="{{ $ico->name }}" alt="{{ $ico->name }}" style="width: 100%;border-bottom: 1px solid #eee;min-height: 180px;">
                @else
                <img class="img-fluid" src="{{URL::asset('public/images/coins_icons/' . $ico->coin_image)}}" title="{{ $ico->name }}" alt="{{ $ico->name }}" style="width: 100%;border-bottom: 1px solid #eee;min-height: 180px;">
                @endif
                <div class="card-body">
