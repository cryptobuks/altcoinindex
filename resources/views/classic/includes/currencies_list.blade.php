...
<div class="btn-group" style="z-index:99999999">
    <div class="ui floating dropdown labeled icon button"><span class="text selected-currency"> @if(isset($_COOKIE['SELECTED_CURRENCY'])) {{$_COOKIE['SELECTED_CURRENCY']}} @else USD @endif </span>
        <div class="menu" style="z-index:99999999">
            ...
            <div class="scrolling menu top-currency-dropdown"> 
	            ...
	            @foreach($currencies_list as $currency)
                	<div class="item @if(isset($_COOKIE['SELECTED_CURRENCY'])) @if($_COOKIE['SELECTED_CURRENCY'] === $currency->currency)selected @endif @endif" value="{{$currency->rate}}" id="{{$currency->icon}}" rel="{{$currency->currency}}"><i class="{{ $currency->flag }} flag"></i>{{$currency->currency}}</div>
                @endforeach
            </div>
        </div>
    </div>
