...
use App\{CryptoMarkets, CryptoCoinsFullDetails, FavoritesCoins, CryptoHistoricalDayData, CryptoExchangesVolume, Affiliates,CryptoNews,CustomWallets};
use DB, URL, Auth;
class CryptoCurrenciesController extends Controller
{
	...
	public function index($alias = '')
	{
		if ($alias !== '') {
			...
			if(isset($market->symbol)) {
				$coin_details = CryptoCoinsFullDetails::where('symbol', '=', $market->symbol)
									->first();
				return view(getCurrentTemplate() . '.pages.coin_details', [
					'market'=> $market,
					'coin_details'=> $coin_details,
					'favorite_coins' => $this->getFavoriteCoins(), 
					'object_details'=> new CryptoCurrenciesController,
	...
	public function defi()
	{
		...
		$data = [
			...
			'streaming_data' => trim(implode(',', $streaming_data), ','), 
			'affiliates'=> Affiliates::all(),
			'sub_heading' => __('v9.DEFI_SUB_HEADING'),
			'title' => __('v9.DEFI_TITLE'),
			'desc' => __('v9.DEFI_DESCRIPTION'),
			'favorite_coins' => $this->getFavoriteCoins()
		];
		return view(getCurrentTemplate() . '.pages.defi', $data);
	}

	public function topCurrencies()
	{
		$crypto_top_markets = CryptoMarkets::where('status', '=', 1)
								->where('rank', '>', 0)
								->orderBy('rank', 'asc')->simplePaginate(50);
		$data = [
	...
	public function topGainersCurrencies()
	{
		$crypto_top_markets = CryptoMarkets::where('status', '=', 1)
								->where('rank', '>', 0)
								->where('percent_change_day', '>', 0)
								->orderBy('percent_change_day', 'desc')->simplePaginate(50);
		$data = [
			'crypto_top_markets' => $crypto_top_markets, 
			'favorite_coins' => $this->getFavoriteCoins(), 
			'affiliates'=> Affiliates::all(),
	...
	public function topLosersCurrencies()
	{
		$crypto_top_markets = CryptoMarkets::where('status', '=', 1)
								->where('rank', '>', 0)
								->where('percent_change_day', '<', 0)
								->orderBy('percent_change_day', 'asc')->simplePaginate(50);
		$data = [
			'crypto_top_markets' => $crypto_top_markets, 
			'favorite_coins' => $this->getFavoriteCoins(), 
			'affiliates'=> Affiliates::all(),
	...
	public function allTimeHighLowCurrencies()
	{
		...
		$markets = CryptoMarkets::where('status', '=', 1)
					->where('rank', '>', 0)
					->orderBy('rank', 'asc')->simplePaginate(50);
	...
	public function getCoinsLatestHighLow($coins_ids)
	{
		...
		DB::raw("SELECT coin, low AS latest_low, close AS latest_high
			FROM crypto_historical_day_datas 
			WHERE id IN (".$ids.")") 
		);
	...
	public function getCustomWallets($coin = '') 
	{
		return [
			'custom_wallets' => CustomWallets::where('type', '=', 'custom')->where('coin', '=', $coin)->get(),
			'all_wallets' => CustomWallets::where('type', '=', 'all')->get()
		];
	}
}
