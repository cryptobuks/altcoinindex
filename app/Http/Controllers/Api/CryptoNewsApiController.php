...
class CryptoNewsApiController extends Controller
{
	...
	public function newsApiNews()
	{
		...
		$news_feed = json_decode(CurlCallController::curl('https://newsapi.org/v2/top-headlines?q=bitcoin&apiKey=' . $apiKey), true);
		if(isset($news_feed['articles'])) {
	...
	{
		$news_feed = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/news/?lang=EN&api_key=' . $this->key), true);
