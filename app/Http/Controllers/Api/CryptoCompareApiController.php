...
use App\{CryptoTopPairs, CryptoMarkets, CryptoHistoricalDayData, CryptoHistoricalHourData, CryptoCoinsFullDetails, CryptoCoinsRates, CryptoMiningEquipments, CryptoExchangesVolume, MiningPools, Wallets, Exchanges};
...
class CryptoCompareApiController extends Controller
{
	...
	public function __construct()
	{
        ...
        $this->key = getCryptoCompareAPI();
	}

    public function getExchanges()
    {
        $exchanges = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/exchanges/general?api_key=' . $this->key), true);
        if($exchanges['Response'] == 'Success') {
            foreach ($exchanges['Data'] as $exchange) {
                $data[] = $this->prepareExchanges($exchange);
            }
        }
        if(isset($data) && count($data) > 0) {
            BulkInsertUpdate::do($this->exchanges->getTable(), $data);
        }
    }

    public function prepareExchanges($exchange) {
        return [
            'name' => $exchange['Name'],
            'alias' => slugify($exchange['Name']),
            'logo' => $exchange['LogoUrl'],
            'items' => implode(',',$exchange['ItemType']),
            'centralization_type' => $exchange['CentralizationType'],
            'grade_points' => $exchange['GradePoints'],
            'grade' => $exchange['Grade'],
            'grade_points_split' => implode(',',$exchange['GradePointsSplit']),
            'url' => $exchange['AffiliateURL'],
            'country' => $exchange['Country'],
            'order_book' => $exchange['OrderBook'],
            'trades' => $exchange['Trades'],
            'description' => $exchange['Description'],
            'full_address' => $exchange['FullAddress'],
            'fees' => $exchange['Fees'],
            'deposit_methods' => $exchange['DepositMethods'],
            'withdrawal_methods' => $exchange['WithdrawalMethods'],
            'rating' => $exchange['Rating']['Avg'],
            'total_volume_24h' => $exchange['TOTALVOLUME24H']['BTC']
        ];
    }
    ...
    public function getPairs()
	{
		...
		foreach ($markets as $market) {
			...
			$pairs = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/top/pairs?fsym=' . $market_symbol . '&api_key=' . $this->key), true);
	...
	public function getHistoricalDayData($limit = 1)
	{
        for($i=0;$i<250;$i++) {
    		...
    		foreach ($markets as $market) {
    			$market_symbol = $market['symbol'];
    			$response = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/histoday?fsym='.$market_symbol.'&tsym=USD&limit='.$limit.'&aggregate=1&e=CCCAGG&api_key=' . $this->key), true);
    ...
    public function getHistoricalHourData()
    {
        ...
        foreach ($markets as $market) {
            $market_symbol = $market['symbol'];
            $response = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/histohour?fsym='.$market_symbol.'&tsym=USD&limit=3&aggregate=1&e=CCCAGG&api_key=' . $this->key), true);
            if($response['Response'] == 'Success') {
                foreach ($response['Data'] as $histo_data) {
                    if($histo_data['open'] == 0 && $histo_data['close'] == 0 && $histo_data['low'] == 0 && $histo_data['high'] == 0) {
                        continue;
	...
	public function cryptoCoinsRates()
    {
        for($i=0;$i<50;$i++) {
            ...
            $rates = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/pricemultifull?fsyms='.$coins.'&tsyms='.$f_currencies . '&api_key=' . $this->key), true);
            if(isset($rates['RAW'])) {
    ...
    public function getAllExchangesPairs()
    {
        ...
        $exchanges = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/all/exchanges?api_key=' . $this->key), true);
    ...
    public function getExchangesVolumeByPairs()
    {
        ...
        foreach ($pairs as $pair) {
            foreach ($markets as $market) {
                ...
                $exchanges_volume = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/top/exchanges?fsym='.$market->symbol.'&tsym=' . $pair . '&api_key=' . $this->key), true);
	...
	public function cryptoCoinsFullDetails()
    {
        ...
        $coins = json_decode(CurlCallController::curl('https://www.cryptocompare.com/api/data/coinlist/?api_key=' . $this->key), true);
        if (isset($coins['Data'])) {
            foreach ($coins['Data'] as $coin) {
                if(in_array($coin['Symbol'], $symbols)) {
                    continue;
                }
                $details = json_decode(CurlCallController::curl('https://www.cryptocompare.com/api/data/coinsnapshotfullbyid/?id='.$coin['Id'] . '&api_key=' . $this->key), true);
                $social_details = json_decode(CurlCallController::curl('https://www.cryptocompare.com/api/data/socialstats/?id='.$coin['Id'] . '&api_key=' . $this->key), true);
	...
	{
        $equipments = json_decode(CurlCallController::curl('https://www.cryptocompare.com/api/data/miningequipment?api_key=' . $this->key), true);
	...
	public function prepareMiningEquipmentData($details)
	{
        ...
        // $image_name = 'https://www.cryptocompare.com/'.$details['LogoUrl'];
		return [
			'company' => $details['Company'],
			'name' => $details['Name'],
			'alias' => slugify($details['Name']),
			'logo' => $image_name,
			'url' => isset($details['AffiliateURL']) ? $details['AffiliateURL'] : '',
