...
class BlockfolioController extends Controller
{
	public function getAll($user_id = '')
	{
		if($user_id != '') {
			...
			foreach ($blockfolio as $record) {
				...
				$price_data = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/price?fsym='.$pair_coins[1].'&tsyms=USD&api_key=' . getCryptoCompareAPI), true);
	...
	public function getPrice($exchange, $pair)
	{
		...
		$price_data = json_decode(CurlCallController::curl('https://min-api.cryptocompare.com/data/price?fsym='.$pair_coins[0].'&tsyms='.$pair_coins[1].'&e='.$exchange . '&api_key=' . getCryptoCompareAPI), true);
